...
<head>
    ...
    <title><?= isset($pageTitle) ? e($pageTitle) . ' - ' : '' ?>Delivery Portal - Smart Clothing</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .del-navbar { background-color: rgba(30, 41, 59, 0.95); border-bottom: 1px solid var(--border-color); position: sticky; top: 0; z-index: 100; }
        .del-nav-container { max-width: 1200px; margin: 0 auto; padding: 0 24px; height: 68px; display: flex; align-items: center; justify-content: space-between; }
        .del-brand { font-family: 'Outfit', sans-serif; font-size: 20px; font-weight: 700; color: #ffffff; text-decoration: none; display: flex; align-items: center; gap: 8px; }
        .del-brand span { color: var(--accent); }
        .del-nav-links { display: flex; align-items: center; gap: 10px; list-style: none; }
        .del-nav-link { display: inline-flex; align-items: center; gap: 6px; padding: 8px 14px; border-radius: 8px; color: var(--text-muted); text-decoration: none; font-size: 14px; font-weight: 500; transition: all 0.2s ease; }
        .del-nav-link:hover { color: var(--text-main); background-color: rgba(255, 255, 255, 0.05); }
        .del-nav-link.active { color: #ffffff; background-color: var(--accent-light); border: 1px solid rgba(56, 189, 248, 0.3); }
        .del-user-info { display: flex; align-items: center; gap: 12px; padding-left: 16px; border-left: 1px solid var(--border-color); }
        .del-main { max-width: 1200px; margin: 0 auto; padding: 32px 24px 60px; width: 100%; flex: 1; }
        ...
        .badge-pending_assignment { background: rgba(148, 163, 184, 0.15); color: var(--text-muted); border: 1px solid rgba(148, 163, 184, 0.3); }
        .badge-assigned { background: rgba(56, 189, 248, 0.15); color: var(--info); border: 1px solid rgba(56, 189, 248, 0.3); }
        .badge-out_for_delivery { background: rgba(245, 158, 11, 0.15); color: var(--warning); border: 1px solid rgba(245, 158, 11, 0.3); }
        .badge-delivered { background: rgba(16, 185, 129, 0.15); color: var(--success); border: 1px solid rgba(16, 185, 129, 0.3); }
        .badge-returned { background: rgba(239, 68, 68, 0.15); color: var(--danger); border: 1px solid rgba(239, 68, 68, 0.3); }
        .badge-pending { background: rgba(245, 158, 11, 0.15); color: var(--warning); border: 1px solid rgba(245, 158, 11, 0.3); }
        .badge-processing { background: rgba(99, 102, 241, 0.15); color: #a5b4fc; border: 1px solid rgba(99, 102, 241, 0.3); }
        .badge-shipped { background: rgba(56, 189, 248, 0.15); color: var(--info); border: 1px solid rgba(56, 189, 248, 0.3); }
        ...
        .form-control:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(56, 189, 248, 0.25); }
        ...
        .del-footer { border-top: 1px solid var(--border-color); padding: 24px; text-align: center; color: var(--text-muted); font-size: 13px; background: rgba(15, 23, 42, 0.4); }
    </style>
</head>
<body>
<?php
$delUser = \App\Core\Auth::currentUser();
if (!isset($currentPage)) {
    $currentPage = basename($_SERVER['PHP_SELF'] ?? '');
}
?>
<header class="del-navbar">
    <div class="del-nav-container">
        <a href="<?= base_url('delivery/dashboard.php') ?>" class="del-brand">
            Smart<span>Clothing</span> &bull; <small style="font-size: 12px; color: var(--info); text-transform: uppercase; letter-spacing: 0.05em; font-weight: 600;">Delivery</small>
        </a>
        <nav>
            <ul class="del-nav-links">
                <li>
                    <a href="<?= base_url('delivery/dashboard.php') ?>" class="del-nav-link <?= $currentPage === 'dashboard.php' ? 'active' : '' ?>">
                        Dashboard
                    </a>
                </li>
                <li>
                    <a href="<?= base_url('delivery/orders.php') ?>" class="del-nav-link <?= in_array($currentPage, ['orders.php', 'order_detail.php', 'assign_delivery.php']) ? 'active' : '' ?>">
                        Orders
                    </a>
                </li>
                <li class="del-user-info">
                    <span style="font-size: 13px; font-weight: 600; color: #cbd5e1;"><?= e($delUser['name']) ?></span>
